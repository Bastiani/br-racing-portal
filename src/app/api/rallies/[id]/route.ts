...
export async function GET(
  ...
  try {
    const rally = await getRallyById(context.params.id);
    return NextResponse.json(rally);
  } catch (error) {
    console.error(`Erro ao buscar rally com ID ${context.params.id}:`, error);
    return NextResponse.json(
      { error: `Rally com ID ${context.params.id} não encontrado` },
      { status: 404 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    console.error(`Erro ao atualizar rally com ID ${context.params.id}:`, error);
    return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      { success: true, message: `Rally com ID ${context.params.id} excluido com sucesso` }
    );
  } catch (error) {
    console.error(`Erro ao excluir rally com ID ${context.params.id}:`, error);
    return NextResponse.json(
